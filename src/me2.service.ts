...
@Injectable()
export class Me2Service implements OnModuleInit {
  ...
  constructor(
    ...
    this.wallet = Keypair.fromSecretKey(
      bs58.decode(this.configService.get('PRIVATE_KEY3')),
    );
  }
  ...
  onModuleInit() {
    console.log('ME2-Service initialized. Ready to start listening...');
  }
  ...
  async startListening() {
    ...
    walletsToListenFor.forEach(async (account) => {
      try {
        ...
        console.log(
          'Token address ME2-Wallet Instance is listening for ' +
            (await myWalletATA).address,
        );
